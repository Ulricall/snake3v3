...
def mlp(sizes,
    ...
    for i in range(len(sizes) - 1):
        ...
        layers = layers + [nn.Linear(sizes[i], sizes[i + 1]), act]
...
def get_reward(info, snake_index, reward, score):
    ...
    for i in snake_index:
        if score == 1:
            step_reward[i] = step_reward[i] + 50
        elif score == 2:
            step_reward[i] = step_reward[i] - 25
        elif score == 3:
            step_reward[i] = step_reward[i] + 10
        elif score == 4:
            step_reward[i] = step_reward[i] - 5

        if reward[i] > 0:
            step_reward[i] = step_reward[i] + 20
        else:
            self_head = np.array(snake_heads[i])
            dists = [np.sqrt(np.sum(np.square(other_head - self_head))) for other_head in beans_position]
            step_reward[i] = step_reward[i] - min(dists)
            if reward[i] < 0:
                step_reward[i] = step_reward[i] - 10
